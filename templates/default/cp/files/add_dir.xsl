 ...
 <xsl:variable name="message">
   <xsl:choose>
     <xsl:when test="/cp/msgs/msg[@name='not_authorized']">
       <xsl:copy-of select="/cp/strings/cp_msg_file_add_dir_fail" /> &gt;
...
<xsl:template name="content">
    ...
    <table class="formview" border="0" cellspacing="0" cellpadding="0">
     <tr class="title">
      <td colspan="2">
       <xsl:value-of select="/cp/strings/file_action_dir_add"/>
      </td>
     </tr>
     <tr class="instructionrow">
      <td colspan="2">
       <xsl:value-of select="/cp/strings/file_dir_add_title"/>
       &#160;<b><xsl:value-of select="/cp/form/currentDir"/></b>
      </td>
     </tr>
     <tr class="roweven">
      <td class="label">
       <xsl:value-of select="/cp/strings/file_add_dir_name"/>
      </td>
      <td class="contentwidth">
       <input name="newDirName" value="" size="35">
         <xsl:if test="/cp/form/newDirName">
           <xsl:attribute name="value">
             <xsl:value-of select="/cp/form/newDirName" />
           </xsl:attribute>
         </xsl:if>
        </input>
      </td>
     ...
     <tr class="controlrow">
      <td colspan="2">
       ...
       <input class="floatright" type="submit" name="ok" value="{/cp/strings/file_btn_ok}" onClick="actionType.value='_addDir'" />
      </td>
     </tr>
    </table>
