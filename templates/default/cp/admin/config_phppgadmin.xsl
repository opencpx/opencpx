...
<xsl:variable name="numAllow">
  <xsl:value-of select="count(/cp/vsap/vsap[@type='app:phppgadmin:status']/allow_from)"/>
</xsl:variable>
...
<xsl:template match="/">
  <xsl:call-template name="bodywrapper">
    <xsl:with-param name="title"><xsl:copy-of select="/cp/strings/cp_title" /> : <xsl:copy-of select="/cp/strings/bc_system_config_phppgadmin_settings" /></xsl:with-param>
    <xsl:with-param name="formaction">config_phppgadmin.xsl</xsl:with-param>
    ...
    <xsl:with-param name="breadcrumb">
      <breadcrumb>
        ...
        <section>
          <name><xsl:copy-of select="/cp/strings/bc_system_config_phppgadmin_settings" /></name>
          <url>#</url>
...
<xsl:template name="content">
        ...
        <table class="formview" border="0" cellspacing="0" cellpadding="0">
          <tr class="title">
            <td colspan="2"><xsl:copy-of select="/cp/strings/phppgadmin_access_control_title" /></td>
          </tr>
          <tr class="instructionrow">
            <td colspan="2"><xsl:value-of select="/cp/strings/phppgadmin_access_control_instr"/></td>
          </tr>
          <tr class="roweven">
            <td class="label"><xsl:value-of select="/cp/strings/access_control_label"/></td>
            <td>
              <xsl:choose>
                <xsl:when test="/cp/vsap/vsap[@type='app:phppgadmin:status']/allow_from_all = 'yes'">
                  <xsl:value-of select="/cp/strings/phppgadmin_access_control_allow_from_all"/><br/>
                  <br/>
                ...
                <xsl:otherwise>
                  <xsl:value-of select="/cp/strings/phppgadmin_access_control_allow_from_list"/><br/>
                  <br/>
                  <select name="allow_list" size="12" multiple="multiple">
                    <xsl:for-each select="/cp/vsap/vsap[@type='app:phppgadmin:status']/allow_from">
       ...
       <table class="formview" border="0" cellspacing="0" cellpadding="0">
          <tr class="title">
            <td colspan="2"><xsl:copy-of select="/cp/strings/phppgadmin_postgresql_password_title" /></td>
          </tr>
          <tr class="instructionrow">
            <td colspan="2"><xsl:value-of select="/cp/strings/phppgadmin_postgresql_password_instr"/></td>
          </tr>
          <tr class="rowodd">
