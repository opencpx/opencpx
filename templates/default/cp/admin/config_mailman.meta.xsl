      ...
      <xsl:when test="string(/cp/vsap/vsap[@type='error'])">
        <xsl:choose>
          <xsl:when test="/cp/vsap/vsap[@type='error']/code = 100">
            <xsl:call-template name="set_message">
              <xsl:with-param name="name">mailman_settings_error_password_missing</xsl:with-param>
            </xsl:call-template>
          </xsl:when>
          <xsl:when test="/cp/vsap/vsap[@type='error']/code = 101">
            <xsl:call-template name="set_message">
              <xsl:with-param name="name">mailman_settings_error_password_mismatch</xsl:with-param>
            </xsl:call-template>
          </xsl:when>
          <xsl:when test="/cp/vsap/vsap[@type='error']/code = 102">
            <xsl:call-template name="set_message">
              <xsl:with-param name="name">mailman_settings_password_change_error</xsl:with-param>
            </xsl:call-template>
          </xsl:when>
          ...
          <xsl:otherwise>
            <xsl:call-template name="set_message">
              <xsl:with-param name="name">mailman_settings_failure</xsl:with-param>
            </xsl:call-template>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:when>
      <xsl:otherwise>
        <xsl:choose>
          <xsl:when test="string(/cp/form/save) and string(/cp/form/new_password)">
            <xsl:call-template name="set_message">
              <xsl:with-param name="name">mailman_settings_success_both</xsl:with-param>
            </xsl:call-template>
          </xsl:when>
          <xsl:when test="string(/cp/form/save)">
            <xsl:call-template name="set_message">
              <xsl:with-param name="name">mailman_settings_success</xsl:with-param>
            </xsl:call-template>
          </xsl:when>
          <xsl:when test="string(/cp/form/new_password)">
            <!-- only changed the password -->
            <xsl:call-template name="set_message">
              <xsl:with-param name="name">mailman_settings_password_change_success</xsl:with-param>
            </xsl:call-template>
            <redirect>
              <path>cp/admin/services.xsl</path>
            </redirect>
