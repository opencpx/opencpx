      ...
      <xsl:with-param name="vsap">
        <vsap>
          <xsl:if test="$saveConfig = '1'">
            <vsap type="app:phpmyadmin:config">
              <xsl:choose>
          ...
          <vsap type="app:phpmyadmin:status" />
        </vsap>
      </xsl:with-param>
    </xsl:call-template>

    <xsl:if test="string(/cp/form/save)">
      <xsl:if test="/cp/vsap/vsap[@type='app:phpmyadmin:config']/status = 'ok'">
        <xsl:call-template name="set_message">
          <xsl:with-param name="name">phpmyadmin_mysql_password_change_success</xsl:with-param>
        </xsl:call-template>
      </xsl:if>
    </xsl:if>
    <xsl:choose>
