...
<xsl:template match="/">
  <xsl:call-template name="bodywrapper">
    <xsl:with-param name="title"><xsl:copy-of select="/cp/strings/cp_title" /> : <xsl:copy-of select="/cp/strings/bc_system_config_mailman_settings" /></xsl:with-param>
    <xsl:with-param name="formaction">config_mailman.xsl</xsl:with-param>
    ...
    <xsl:with-param name="breadcrumb">
      <breadcrumb>
        ...
        <section>
          <name><xsl:copy-of select="/cp/strings/bc_system_config_mailman_settings" /></name>
          <url>#</url>
...
<xsl:template name="content">
        ...
        <input type="hidden" name="application" value="mailman" />

        <table class="formview" border="0" cellspacing="0" cellpadding="0">
          <tr class="title">
            <td colspan="2"><xsl:copy-of select="/cp/strings/mailman_settings_title" /></td>
          </tr>
          <tr class="instructionrow">
            <td colspan="2"><xsl:value-of select="/cp/strings/mailman_settings_instr"/></td>
          </tr>
          <tr class="rowodd">
            <td class="label"><xsl:value-of select="/cp/strings/mailman_password_new_password"/></td>
          ...
          <tr class="roweven">
            <td class="label"><xsl:value-of select="/cp/strings/mailman_password_confirm_password"/></td>
            ...
            <td class="appstatcolumn" colspan="2">
              ...
              <xsl:if test="$num_backups_available > 0">
                <span class="floatright">
                  <a href="{$base_url}/cp/admin/config_file_restore.xsl?application=mailman&amp;config_path={$config_path}">
