      ...
      <xsl:with-param name="vsap">
        <vsap>
          <vsap type="sys:ssl:cert_install">
            ...
            <key><xsl:value-of select="/cp/form/key" /></key>
            <cert><xsl:value-of select="/cp/form/cert" /></cert>
            <cacert><xsl:value-of select="/cp/form/cacert" /></cacert>
  ...
  <xsl:choose>
    <xsl:when test="/cp/vsap/vsap[@type='sys:ssl:cert_install']/status = 'ok'">
      <xsl:call-template name="set_message">
        <xsl:with-param name="name">domain_cert_success</xsl:with-param>
      </xsl:call-template>
      <redirect>
        <path>cp/domains/index.xsl</path>
      </redirect>
    ...
    <xsl:when test="/cp/vsap/vsap[@type='error']/code = 107">
      <xsl:call-template name="set_message">
        <xsl:with-param name="name">domain_cert_error_key_file</xsl:with-param>
      </xsl:call-template>
    </xsl:when>
    <xsl:when test="/cp/vsap/vsap[@type='error']/code = 108">
      <xsl:call-template name="set_message">
        <xsl:with-param name="name">domain_cert_error_cacert_file</xsl:with-param>
      </xsl:call-template>
    </xsl:when>
    <xsl:when test="/cp/vsap/vsap[@type='error']/code = 109">
      <xsl:call-template name="set_message">
        <xsl:with-param name="name">domain_cert_error_cert_mismatch</xsl:with-param>
      </xsl:call-template>
    </xsl:when>
