...
## $SMEId: apps/vsap/modules/VSAP-Server-Modules-vsap-web-rss/t/04_post.t,v 1.2.2.1 2006/04/28 14:58:07 kwhyte Exp $
...
## ---------------------------------------------------------------------- 
## test web:rss:post:feed - post feed; single
## ---------------------------------------------------------------------- 
...
undef $resp;
$resp = $client->xml_response(qq!<vsap type="web:rss:post:feed"><ruid>$ruid</ruid></vsap>!);

ok( ! $resp->findnodes('/vsap/vsap[@type="error"]'), "There was no error.");
...
## ----------------------------------------------------------------------
## test web:rss:post:feed - post feed; exceptions
## ----------------------------------------------------------------------

## test web:rss:post:feed: error, ruid required

undef $resp;
$resp = $client->xml_response(q!<vsap type="web:rss:post:feed"/>!);

SKIP: {
    ($node) = $resp->findnodes('/vsap/vsap[@caller="web:rss:post:feed"]');
    skip "unable to obtain node from feed listing", 1
        unless ok( $node,"found feed node" );
    is( $node->findvalue('./code'), '102', "ruid required as expected" );
}

## test web:rss:post:feed: error, ruid not found

undef $resp;
$resp = $client->xml_response(q!<vsap type="web:rss:post:feed"><ruid>0123456789</ruid></vsap>!);
