...
sub _get_rhel_pkg_info
{
    ...
    if ($options{'package'}) {
        ...
        if ($info =~ /is not installed/) {
            ...
            @command = ('/usr/bin/yum', 'info', $options{'package'});
            my @results = `@command`;
            my ($pkgname, $lastkey);
            $pkgname = $lastkey = "";
            ...
            foreach my $field (@infoqf) {
                if ($info =~ m#<Package name=\"$pkgname\">.*?<$field>(.*?)</$field>#s) {
                    my $value = $1;
                    my $key = $field;
                    $key =~ tr/A-Z/a-z/;
                    my @values = split(/\n/, $value);
            ...
            foreach my $line (@results) {
                if ($line =~ /(.*?)\-.*/) {
                    push(@{$pkginfo{$options{'package'}}->{'requiredby'}}, $1);
                }
            }
            # get available updates
            my @updates = ();
            @command = ('/usr/bin/yum', 'list', 'updates', $options{'package'});
            @results = `@command`;
            my $available = 0;
            foreach my $line (@results) {
            ...
            @{$pkginfo{$options{'package'}}->{'updates'}} = @updates;
        }
        # get dependencies for specified package
        my %dependencies = ();
        @command = ('/usr/bin/yum', '-C', 'deplist', $options{'package'});
        my @results = `@command`;
        my ($pkgname, $lastkey);
        $pkgname = $lastkey = "";
        foreach my $line (@results) {
        ...
        @command = ('/usr/bin/yum', '-C', 'list', keys(%pkginfo));
        @results = `@command`;
        my $installed = 0;
        my $available = 0;
        foreach my $result (@results) {
...
sub handler
{
    ...
    if ($installed || $group) {
        ...
        %_psortprefs = ( package_list_sortby  => 'name',
                         package_list_order   => 'ascending',
                         package_list_sortby2 => 'installtime',
                         package_list_order2  => 'ascending' );
    }
    else {
        # only thing to sort on is package name, so just need order
        %_psortprefs = ( package_search_order  => 'ascending' );
    }
    ...
    if ($installed) {
        $root->appendTextChild( sortby    => $_psortprefs{'package_list_sortby'} );
        $root->appendTextChild( order     => $_psortprefs{'package_list_order'} );
        $root->appendTextChild( sortby2   => $_psortprefs{'package_list_sortby2'} );
        $root->appendTextChild( order2    => $_psortprefs{'package_list_order2'} );
    }
    else {
        $root->appendTextChild( order     => $_psortprefs{'package_search_order'} );
    }
    ...
    if ($#sorted_packages > -1) {
        ...
        for my $package ( @sorted_packages[($first_package-1 .. $last_package-1)] ) {
            ...
            # package arch
            $package_node->appendTextChild( arch => $packages{$package}->{'arch'} );
            if ($installed || $group) {
                # package group
