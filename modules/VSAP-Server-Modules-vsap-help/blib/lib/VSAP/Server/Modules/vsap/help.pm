...
sub get_topics
{
    ...
    elsif (($sysname =~ /freebsd/i) && ($release =~ /^4/)) {
      $platform_type = "freebsd4";
    }
    elsif (($sysname =~ /freebsd/i) && ($release =~ /^6/)) {
      $platform_type = "freebsd6";
    }
...
sub handler
{
    ...
    $case_sensitive = $xml->child('case_sensitive')->value  if ( $xml->child('case_sensitive') );
...
sub handler
{
    ...
    unless( 
        $xml->child('query') &&
        ($query = $xml->child('query')->value) &&
        (length($query) >= $MIN_WORD_SIZE ) 
    )
    {
    ...
    unless(
        $xml->child('category') &&
        ($category = $xml->child('category')->value)
    ) 
    {
    ...
    unless( 
        $xml->child('language') &&
        ($language = $xml->child('language')->value)
    ) 
    {
    ...
    my @topics = VSAP::Server::Modules::vsap::help::get_topics(
        $vsap, 
        [$category], 
        $language,
        $query,
...
sub handler
{
    ...
    my $module = $xmlobj->child('module')
               ? $xmlobj->child('module')->value : '';
    ...
    if ($module) {
        ...
        for (;;) {
            ...
            unless ($pm =~ s/VSAP::Server::Modules::vsap::.*\K::[^:]*$//) {
                $vsap->error($_ERR{HELP_NO_MODULE} =>
    ...
    $root_node->appendTextChild('help' => $help)
        if $help;
